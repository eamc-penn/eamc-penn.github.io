...
A 52 year-old male presents with acute-onset altered mental status and fever. Vitals are notable for BP 90/60 and T 103.4. We can denote the patient as a variable $x$ consisting of all of the relevant attributes of the patient: their HPI, past medical history, current lab values and vitals, etc. Every patient in the world can be expressed in terms of these features - we can write the set of all possible patients in the world as $\mathcal{X}$ (a set which is obviously incredibly large). $x\sim \mathcal{X}$, which means that our particular 52 year-old patient in front of us is *sampled from* the population $\mathcal{X}$ of all patients.

On our first day as a medical student, we might not know what to do with this patient. Do we admit them and start them on IV antibiotics? Do we call a neurology consult? Do we just send the patient home? Each of these clinical interventions can be thought of as an action $a$, which is similarly sampled from the space of all possible actions $\mathcal{A}$ that we can take to try to help the patient get better.

After observing a patient $x$ and performing an action $a$, we monitor the patient to see if they improve. The patient's outcome can be denoted as a variable $y$ (for example, $y=0$ if the patient deteriorates and $y=1$ if the patient gets better). We observe the clinical outcome $y$ sampled from the set of all possible outcomes $\mathcal{R}$.

Over the course of medical school, we see hundreds if not thousands of tuples $(x, a, y)$ sampled from $\mathcal{X}\times\mathcal{A}\times\mathcal{R}$ through clerkships, sub-Is, exams, and UWorld, and use this dataset of patient-outcome pairs to learn hard-to-write-down algorithms for choosing the best clinical intervention $a$ given a patient $x$.
